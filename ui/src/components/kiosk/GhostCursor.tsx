...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  const fadeDelay = fadeDelayMs ?? (isTouch ? 300 : 400);
  const fadeDuration = fadeDurationMs ?? (isTouch ? 500 : 700);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (pointerActiveRef.current) {
        ...
        const idleX = 0.5 + Math.sin(t * 1.3) * 0.18 + Math.cos(t * 0.7) * 0.07;
        const idleY = 0.5 + Math.cos(t * 1.5) * 0.18 + Math.sin(t * 0.5) * 0.07;
        velocityRef.current.set(
          idleX - mat.uniforms.iMouse.value.x,
          idleY - mat.uniforms.iMouse.value.y
        );
        mat.uniforms.iMouse.value.lerp(new THREE.Vector2(idleX, idleY), 0.08);
      ...
      if (!pointerActiveRef.current && !autoIdle && fadeOpacityRef.current <= 0.001) {
